...
/// @brief GPIO port used for the virtual COM port (VCOM) TX and RX pins.
static constexpr GPIO_Port_TypeDef VCOM_PORT = gpioPortA;

/// @brief GPIO pin number for VCOM TX (PA8).
static constexpr uint32_t VCOM_TX = 8u;

/// @brief GPIO pin number for VCOM RX (PA9).
static constexpr uint32_t VCOM_RX = 9u;
...
void hal_uart_init(uint32_t baud) {
  ...
  GPIO->EUSARTROUTE[1].TXROUTE =
      (VCOM_PORT << _GPIO_EUSART_TXROUTE_PORT_SHIFT) |
      (VCOM_TX << _GPIO_EUSART_TXROUTE_PIN_SHIFT);
  GPIO->EUSARTROUTE[1].RXROUTE =
      (VCOM_PORT << _GPIO_EUSART_RXROUTE_PORT_SHIFT) |
      (VCOM_RX << _GPIO_EUSART_RXROUTE_PIN_SHIFT);
  GPIO->EUSARTROUTE[1].ROUTEEN =
      GPIO_EUSART_ROUTEEN_TXPEN | GPIO_EUSART_ROUTEEN_RXPEN;
}

void hal_uart_putchar(char c) {
  EUSART_Tx(EUSART1, static_cast<uint8_t>(c));
}
